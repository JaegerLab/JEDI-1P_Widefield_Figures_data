...
%
% 'v' - false [default] if true the median value is highlighted
...
params.addParameter('style','patch', @(x) ischar(x) && any(strncmpi(x,{'patch','line','sdline', 'errorBar'},4)) );
...
params.addParameter('ColorScale', @(x) ischar(x) && any(strncmpi(x,{'blueScale','grayScle'},4)) );
...
colorScale = params.Results.ColorScale;
...
if strcmp(colorScale, 'blueScale')
    meanBarColor = [0, 0, 0];
    dataPoints = [0.3010 0.7450 0.9330];
elseif strcmp(colorScale, 'darkBlueScale')
    meanBarColor = [0, 0, 0];
    dataPoints = [0 0.4470 0.7410];    
elseif strcmp(colorScale, 'purpleScale')
    meanBarColor = [0, 0, 0];
    dataPoints = [0.4940 0.1840 0.5560];    
elseif strcmp(colorScale, 'orangeScale')
    meanBarColor = [0, 0, 0];
    dataPoints = [0.9290 0.6940 0.1250];    
elseif strcmp(colorScale, 'redScale')
    meanBarColor = [0, 0, 0];
    dataPoints = [0, 0, 0.4];    
elseif strcmp(colorScale, 'darkRedScale')
    meanBarColor = [0, 0, 0];
    dataPoints = [0.6350 0.0780 0.1840];  
elseif strcmp(colorScale, 'grayScale')
    meanBarColor = [0, 0, 0];
    dataPoints = [0.6, 0.6, 0.6];
end
...
function [h,statsOut]=myPlotter(X,Y)
    ...
    for k=1:length(X)
        ...
        if strcmp(style,'patch') || strcmp(style,'sdline')
            h(k).semPtch=patchMaker(SEM(k),[0.3010 0.7450 0.9330]);
            h(k).mu=plot([X(k)-jitScale,X(k)+jitScale],[mu(k),mu(k)],'LineStyle', '-', 'Color', [0 0.4470 0.7410],...
                'linewidth',1);
            if markMedian
                statsOut(k).median = med(k);
                h(k).med=plot([X(k)-jitScale,X(k)+jitScale],[med(k),med(k)],':', 'Color', [0 0.4470 0.7410],...
                    'linewidth',1);
            end
        end
        
        % Only plot SEM as error bar; no SD plot
        if strcmp(style, 'errorBar') 
            h(k).mu=plot([X(k)-jitScale*1.1,X(k)+jitScale*1.1],[mu(k),mu(k)],'LineStyle', '-', 'Color', meanBarColor,...
                'linewidth',1);
            h(k).errorBar = errorBarMaker(SEM(k), [0, 0, 0]);
            uistack(h(k).mu,'top')
            uistack(h(k).errorBar,'top')
        end
        
        % Generate scatter in X
        thisX=violaPoints(thisX,thisY);
        C=cols(k,:);
        h(k).data=scatter(thisX, thisY, '.', 'MarkerEdgeColor', dataPoints); 
        h(k).data.MarkerEdgeAlpha = 0.6;
        h(k).data.SizeData = 8;
    ...
    function ptch=patchMaker(thisInterval,tColor)
        ...
        ptch=patch([X(k)-jitScale, X(k)+jitScale, X(k)+jitScale, X(k)-jitScale],...
                [l,l,u,u], 0);
        set(ptch,'edgecolor','none','facecolor',tColor, 'facealpha',0.4)
    end %function patchMaker


    function errorBar = errorBarMaker(thisInterval, tColor)
        errorBar = errorbar(X(k), mu(k),thisInterval, 'Color', tColor, 'lineWidth', 1);
    end
    ...
    function X=violaPoints(X,Y)
        ...
        for jj=1:length(inds)
            tWidth = jitter/4 * (1-exp(-0.1 * (counts(jj)-1)));
